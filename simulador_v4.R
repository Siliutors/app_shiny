...
source("//filer2usr/dsm/dsm_comun/SINRE simulador/Shiny/mix_plot.R")

 # rutas.fichero <- as.data.frame(read_excel("//filer2usr/dsm/dsm_comun/SINRE simulador/Shiny/fichero_rutas.xlsx", col_names = T))
 # fecha1 = "2021-09-27"
 # fecha2 = "2021-09-28"
 # fechas = c(fecha1,fecha2)
 # nombre = "test"
 # dir = "//filer2usr/dsm/dsm_comun/SINRE simulador/RESULTADOS"
 # pmin.sol <- 2
 # pmin.ts <- 2
 # pmin.eol <- 2
 # PT_pmin.sol <- 2
...
algoritmo <-function(  rutas.fichero,
           fechas,
           fecha1,
           fecha2,
           nombre,
...
##Adapt vars
  ...
  fechas <- as.numeric(as.Date(fechas[1])):as.numeric(as.Date(fechas[2]))
  fechas <- as.Date(fechas, origin = "1970-01-01")
  
 
  
  for (k in seq(1,length(fechas))){

    print(paste("Vamos por la fecha", k, "de", length(fechas), ". Tiempo estimado:",((length(fechas))-k)*0.5/60, "horas" ))
    message(print(paste("Vamos por la fecha", k, "de", length(fechas), ". Tiempo estimado:",((length(fechas))-k)*0.5/60, "horas" )))
  
  
    
    if (as.numeric(format(as.Date(fechas[k]),"%m")) == 2 & as.numeric(format(as.Date(fechas[k]),"%d")) == 29){
      
      next
    }

    wd1 = as.numeric(format(as.Date(fechas[k]),"%u"))
    mes1 = as.numeric(format(as.Date(fechas[k]),"%m"))

    fecha.estimacion = format(as.Date(fechas[k]),"%d/%m/%Y")
    
    
    ccinput <-   cCCmodel[which(as.Date(cCCmodel$FECHA,"%d/%m/%Y") == as.Date(fechas[k])
                                & cCCmodel$HORA == 1 ),]$VALOR
                         
    cainput <-   cCAmodel[which(as.Date(cCAmodel$FECHA,"%d/%m/%Y") == as.Date(fechas[k])
                              & cCAmodel$HORA == 1 ),]$VALOR

    demandainput<- demanda.ES[which(as.Date(demanda.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]

    co2input<- co2[which(as.Date(co2$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    gasinput<- gas[which(as.Date(gas$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    api2input<- api2[which(as.Date(api2$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    
    camaxinput<- camax.ES[which(as.Date(camax.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    ccmaxinput<- ccmax.ES[which(as.Date(ccmax.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    remaxinput<- cogmax.ES[which(as.Date(cogmax.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    
    hidrofluyeinput<- previ.HidroFluye.ES[which(as.Date(previ.HidroFluye.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    numaxinput<- previ.NK.ES[which(as.Date(previ.NK.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    fvinput <-  previ.FV.ES[which(as.Date(previ.FV.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    tsolarinput <-  previ.TS.ES[which(as.Date(previ.TS.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    eolinput <-  previ.EO.ES[which(as.Date(previ.EO.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]	
    mhinput <-  previ.minihidro.ES[which(as.Date(previ.minihidro.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    eolinput <-  previ.EO.ES[which(as.Date(previ.EO.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]	
    ptinput <- capinterPT[which(as.Date(capinterPT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    frinput <- capinterFR[which(as.Date(capinterFR$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]	
    futurosfrinput <- futurosFR[which(as.Date(futurosFR$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    tfijoinput <- tfijo[which(as.Date(tfijo$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    hidroUGH.ESinput <- previ.HidroUGH.ES[which(as.Date(previ.HidroUGH.ES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    ofebombeoESinput <- ofebombeoES[which(as.Date(ofebombeoES$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    capbateriasinput <- capmax.bat[which(as.Date(capmax.bat$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    ofebateriasinput <- ofebaterias[which(as.Date(ofebaterias$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    #PT
    PTdemandinput = demanda.PT[which(as.Date(demanda.PT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    PTfvinput = previ.FV.PT[which(as.Date(previ.FV.PT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    PTeolinput = previ.EO.PT[which(as.Date(previ.EO.PT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    PTfluyehidroinput = previ.HidroFluye.PT[which(as.Date(previ.HidroFluye.PT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    PTcamaxinput = camax.PT[which(as.Date(camax.PT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    PTccmaxinput = ccmax.PT[which(as.Date(ccmax.PT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    hidroUGH.PTinput <- previ.HidroUGH.PT[which(as.Date(previ.HidroUGH.PT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    ofebombeoPTinput <- ofebombeoPT[which(as.Date(ofebombeoPT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
    cogmax.PTinput <- cogmax.PT[which(as.Date(cogmax.PT$FECHA,"%d/%m/%Y") == as.Date(fechas[k])),]
     #/PT      
    pb <- progress_bar$new(total = 24, width = 60)
    for (j in c(1:24)){
      print(pb$tick())
  ...
  write.table(fores,paste(dir_guardar, "/precios_mix_", nombre, ".csv",sep = ""),
              row.names = F, sep = ";", dec = ",", col.names = T, append = T)

  
  #rm(list = ls())
  message(print(paste("Fin, ejecutado correctamente.",timestamp())))
  return(fores)
